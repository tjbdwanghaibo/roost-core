...
| `service/match`、`service/mail`、`service/session`、`servicemetrics` | 通用服务的**领域实现**（匹配——队列 / 票据 / 原子 Commit 的状态机、Redis store、`Grouping` 策略接口与两个实现、错误码段）与服务计数契约 `Reporter`；邮件——信封 / 邮箱状态机 / 三段式领取 / Redis stores（M-07）；会话——幂等 Enter / 每 owner 一个活 run / 资源恰好释放一次 / `Admin` 操作面（M-08）；Mod、RPC 传输与 ClientMod 在 roost-kit 装配 | 自己的匹配服务进程；或在游戏进程里写 matchmaker（Candidates → Grouping → Commit） |
