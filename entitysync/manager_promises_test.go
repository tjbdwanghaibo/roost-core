...
type decoded struct {
	...
	objects map[int64]core.ObjectOperation
	updates map[int64]entity.SubjectSyncUpdate
}

func decodeFrame(t *testing.T, data []byte) decoded {
	t.Helper()
	frame, err := DecodeFrame(data, core.DefaultLimits())
	if err != nil {
		t.Fatalf("decode frame: %v", err)
	}
	out := decoded{frame: frame, objects: make(map[int64]core.ObjectOperation), updates: make(map[int64]entity.SubjectSyncUpdate)}
	refs := make(map[core.ObjectRef]int64)
	for _, object := range frame.Objects {
		...
		if object.Operation == core.ObjectRemove {
			...
			out.objects[-int64(object.Ref.ID)] = core.ObjectRemove
		}
	}
	return out
}
...
func TestEachSessionGetsASnapshotThenDeltasAndOnePackServesThemAll(t *testing.T) {
	...
	for _, session := range []SessionID{1, 2} {
		frame := oneFrame(t, transport, session)
		if frame.frame.Kind != core.FrameFull || frame.objects[1001] != core.ObjectCreate || !frame.updates[1001].Full {
			t.Fatalf("session %d first frame is not a snapshot create: kind=%v objects=%v", session, frame.frame.Kind, frame.objects)
		}
	}
	if packs != 1 {
	...
	for _, session := range []SessionID{1, 2} {
		frame := oneFrame(t, transport, session)
		update := frame.updates[1001]
		if frame.frame.Kind != core.FrameDelta || frame.frame.BaseTick != 1 || frame.objects[1001] != core.ObjectUpdate || update.Full || update.Mask != 0x4 {
			t.Fatalf("session %d second frame is not a delta from the snapshot: kind=%v base=%d objects=%v update=%+v", session, frame.frame.Kind, frame.frame.BaseTick, frame.objects, update)
		}
...
func TestAFailingSessionIsClosedAloneAndTheOthersStillReceive(t *testing.T) {
	...
	for _, session := range []SessionID{1, 3} {
		frame := oneFrame(t, transport, session)
		if frame.objects[1002] != core.ObjectUpdate {
			t.Fatalf("session %d did not receive the delta after another session failed: %v", session, frame.objects)
		}
	}
...
func TestRetryLaterAbandonsTheTickWithoutBlamingAnySession(t *testing.T) {
	...
	mustFlush(t, manager)
	for _, session := range []SessionID{1, 2} {
		if frame := oneFrame(t, transport, session); frame.objects[1003] != core.ObjectCreate {
			t.Fatalf("session %d did not get its snapshot once the transport came back: %v", session, frame.objects)
		}
	}
}
...
func TestUnsubscribeAndUnregisterOweARemoveOnlyToWhoHoldsTheObject(t *testing.T) {
	...
	if len(frame.frame.Objects) != 1 || frame.frame.Objects[0].Operation != core.ObjectRemove {
		t.Fatalf("session 1 did not receive an ObjectRemove: %+v", frame.frame.Objects)
	}
	...
	mustFlush(t, manager)
	if frame := oneFrame(t, transport, 1); len(frame.frame.Objects) != 1 || frame.frame.Objects[0].Operation != core.ObjectRemove {
		t.Fatalf("retirement did not tell the holder the object is gone: %+v", frame.frame.Objects)
	}
...
func TestCloseSessionForgetsItsSubscriptionsWithoutAFrame(t *testing.T) {
	...
	if frame := oneFrame(t, transport, 1); frame.objects[1005] != core.ObjectUpdate {
		t.Fatalf("the remaining session did not get the delta: %v", frame.objects)
	}
}
...
func TestTheDurabilityGateHoldsTheWholeSubjectUntilTheWatermarkReachesIt(t *testing.T) {
	...
	mustFlush(t, manager)
	frame := oneFrame(t, transport, 1)
	if frame.objects[1006] != core.ObjectCreate || !frame.updates[1006].Full || frame.updates[1006].Version != 1 {
		t.Fatalf("once durable, the session did not get a snapshot at the committed version: %v %+v", frame.objects, frame.updates[1006])
	}
	if state.PendingDirty() {
...
func TestChangingTheProfileResendsAFullSnapshotOnTheSameObject(t *testing.T) {
	...
	if second.objects[1007] != core.ObjectUpdate || !second.updates[1007].Full || second.updates[1007].Profile.Key != "far" {
		t.Fatalf("profile change did not resend a full snapshot under the new profile: %v %+v", second.objects, second.updates[1007])
	}
	if first.frame.Objects[0].Ref != second.frame.Objects[0].Ref {
		t.Fatalf("the object changed reference across a profile change: %v → %v", first.frame.Objects[0].Ref, second.frame.Objects[0].Ref)
	}
}
...
func TestOneFramePerSessionPerTickSplitOnlyAtTheObjectLimit(t *testing.T) {
	transport := newRecordingTransport()
	manager := newTestManager(t, transport, ManagerConfig{Limits: core.Limits{MaxObjects: 2}})
	packs := 0
	...
	mustFlush(t, manager)
	frame := oneFrame(t, transport, 1)
	if len(frame.frame.Objects) != 2 {
		t.Fatalf("two pending subjects did not share one frame: %+v", frame.frame.Objects)
	}
