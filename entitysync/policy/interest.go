...
//
// spatial.InterestManager is not safe for concurrent use; Interest carries the
// lock that makes it so. Every method is callable from any goroutine.
type Interest struct {
	mu        sync.Mutex
	manager   *entitysync.Manager
	spatial   *spatial.InterestManager
	self      *RelationSource
...
func NewInterest(config InterestConfig) (*Interest, error) {
	...
	manager, err := spatial.NewInterestManager(config.Spatial)
	if err != nil {
		return nil, err
	}
	...
	in := &Interest{
		manager: config.Manager, spatial: manager, session: session, profile: profile,
		relations: make(map[string]*RelationSource), held: make(map[pair]*hold),
	}
	in.sources = []Source{spatialSource{manager: manager}}
...
// spatialSource adapts the core manager to Source. Flush is called with
...
func (in *Interest) Enter(id int64, at spatial.Point) error {
	...
	if err := in.spatial.AddSubject(id, at); err != nil {
		return err
	}
	if err := in.spatial.AddObserver(id, at); err != nil {
		return err
	}
	if in.self != nil {
...
func (in *Interest) Move(id int64, to spatial.Point) error {
	...
	if err := in.spatial.MoveSubject(id, to); err != nil {
		return err
	}
	return in.spatial.MoveObserver(id, to)
}
...
func (in *Interest) Leave(id int64) error {
	...
	if err := in.spatial.RemoveObserver(id); err != nil {
		return err
	}
	return in.spatial.RemoveSubject(id)
}
...
func (in *Interest) Show(id int64, at spatial.Point) error {
	...
	return in.spatial.AddSubject(id, at)
}
...
func (in *Interest) MoveShown(id int64, to spatial.Point) error {
	...
	return in.spatial.MoveSubject(id, to)
}
...
func (in *Interest) Visible(observer int64) []int64 {
	...
	return in.spatial.Visible(observer)
}
...
func (in *Interest) applyEvent(source string, event spatial.InterestEvent, refusals *[]Refusal) {
