...
func main() {
	...
	flag.Parse()
	if flag.NArg() < 1 {
		fail("usage: accountctl [-redis addr] [-prefix key_prefix] upsert-server -sid N -name NAME [-status open|maintenance|full|closed] [-region R]")
	}
	switch flag.Arg(0) {
	case "upsert-server":
		upsertServer(*redisAddr, *redisPassword, *redisDB, *prefix, flag.Args()[1:])
	default:
		fail("unknown command %q (commands: upsert-server)", flag.Arg(0))
	}
}

func upsertServer(redisAddr, password string, db int, prefix string, args []string) {
	...
	service, closeStore := openService(redisAddr, password, db, prefix)
	defer closeStore()
...
func openService(redisAddr, password string, db int, prefix string) (*svcaccount.Service, func()) {
