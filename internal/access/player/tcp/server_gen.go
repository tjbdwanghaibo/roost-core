...
func configFromViper(cfg *viper.Viper) (Config, error) {
	...
	if cfg == nil {
		return result, errors.New("player tcp: config is nil")
	}
	const key = "player_access.tcp."
	result.Enabled = cfg.GetBool(key + "enabled")
	if value := cfg.GetString(key + "addr"); value != "" {
		result.Addr = value
	}
	if value := cfg.GetInt(key + "max_connections"); value != 0 {
		result.MaxConnections = value
	}
	if value := cfg.GetInt(key + "max_connections_per_ip"); value != 0 {
		result.MaxConnectionsPerIP = value
	}
	if value := cfg.GetInt(key + "max_handshakes"); value != 0 {
		result.MaxHandshakes = value
	}
	if value := cfg.GetUint32(key + "max_handshake_bytes"); value != 0 {
		result.MaxHandshakeBytes = value
	}
	if value := cfg.GetUint32(key + "max_payload_bytes"); value != 0 {
		result.MaxPayloadBytes = value
	}
	if value := cfg.GetDuration(key + "handshake_timeout"); value != 0 {
		result.HandshakeTimeout = value
	}
	if value := cfg.GetDuration(key + "idle_timeout"); value != 0 {
		result.IdleTimeout = value
	}
	if value := cfg.GetDuration(key + "write_timeout"); value != 0 {
		result.WriteTimeout = value
	}
	if value := cfg.GetDuration(key + "shutdown_timeout"); value != 0 {
		result.ShutdownTimeout = value
	}
...
func validateConfig(result Config) error {
	...
	if _, _, err := net.SplitHostPort(result.Addr); err != nil {
		return fmt.Errorf("player tcp: invalid addr %q: %w", result.Addr, err)
	}
...
func (mod *Mod) Init(cfg *viper.Viper) error {
	...
	if mod.authenticator == nil {
		...
		if err != nil {
			return fmt.Errorf("player tcp: initialize authenticator: %w", err)
		}
	}
	if mod.authenticator == nil {
		return errors.New("player tcp: authenticator is nil")
	}
	return nil
}
...
func (mod *Mod) Provide(registry *app.Registry) error {
	if registry == nil {
		return errors.New("player tcp: app registry is nil")
	}
	runtime, ok := app.Lookup[*accessplayer.Runtime](registry, accessplayer.Name)
	if !ok || runtime == nil || runtime.Protocols == nil {
		return errors.New("player tcp: player access runtime is unavailable")
	}
	if bound, ok := mod.authenticator.(RegistryBound); ok {
		if err := bound.BindRegistry(registry); err != nil {
			return fmt.Errorf("player tcp: bind authenticator: %w", err)
		}
	}
	mod.runtime = runtime
...
func (mod *Mod) Start() error {
	...
	if mod.transportRuntime != nil {
		mod.transportRuntime.startLifecycle()
	}
...
func (mod *Mod) StopWithContext(ctx context.Context) error {
	if mod.transportRuntime != nil {
		mod.transportRuntime.server.Store(nil)
	}
	if mod.server == nil {
		if mod.transportRuntime != nil {
			mod.transportRuntime.stopLifecycle()
		}
		return nil
	}
...
func (runtime *Runtime) OnSessionClosed(fn func(SessionClosed)) func() {
	...
	if runtime.subscribers == nil {
		runtime.subscribers = make(map[uint64]func(SessionClosed))
	}
...
func (runtime *Runtime) stopLifecycle() {
	if runtime == nil || runtime.lifecycleStop == nil {
		return
	}
	select {
	case <-runtime.lifecycleStop:
...
func (server *Server) readFrameLimit(reader io.Reader, maxPayload uint32) (frame, func(), error) {
	...
	if length > maxPayload {
		return frame{}, func() {}, fmt.Errorf("%w: payload %d exceeds %d", errInvalidFrame, length, maxPayload)
	}
	var payload []byte
	pooled := false
	if length > 0 {
...
func (server *Server) registerSession(current *session) {
	...
	if previous != nil && previous != current {
		_ = previous.Close(gateway.ErrSessionClosed)
	}
}
...
func (server *Server) pushPlayer(ctx context.Context, playerID int64, messageID uint32, payload []byte) error {
	...
	for _, current := range sessions {
		if err := current.push(ctx, messageID, payload); err != nil {
			joined = errors.Join(joined, fmt.Errorf("session %s: %w", current.principal.SessionID, err))
		}
	}
	if joined == nil {
		metrics.IncCounter("player_tcp_push_total", nil, int64(len(sessions)))
	} else {
...
func (session *session) writeFrame(ctx context.Context, flags byte, messageID, sequence uint32, payload []byte) error {
	...
	if session.closeErr != nil {
		return gateway.ErrSessionClosed
	}
...
func (session *session) Close(reason error) error {
	session.closeOnce.Do(func() {
		if reason == nil {
			reason = gateway.ErrSessionClosed
		}
		session.writeMu.Lock()
