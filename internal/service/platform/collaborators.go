...
//
// It is idempotent by construction: the field is keyed by order id, so a
// retried delivery writes the same field again.
...
// grantStore is the one Redis call the deliverer makes, narrowed so a test can
...
func (deliverer *grantDeliverer) Deliver(ctx context.Context, order platform.Order) error {
	...
	if client == nil {
		return fmt.Errorf("purchase delivery: not bound to a registry")
	}
	...
	if !ok {
		...
		return fmt.Errorf("purchase delivery: order %s bought unknown product %q", order.OrderID, order.ProductID)
	}
	...
	if err := client.HSet(ctx, purchase.GrantsKey(prefix, order.PlayerID), order.OrderID, grant); err != nil {
