...
func startGiftSaga(ctx context.Context, registry *app.Registry) (func(), error) {
	...
	// one that owns the player instead of only being refused.
	//
	// Router is core's ownerroute: it resolves the owner from the same table
	// the admission predicate reads, runs the step here when the owner turns
	// out to be us, and otherwise addresses the owner's process directly
	// (`<prefix>.svc.game.<sid>`). A player nobody owns resolves
	// to ErrRouteNotFound and is left alone — claiming an idle player just to
	// have somewhere to send a message is how every process ends up owning
	// every player.
	...
	steps.handoff = &ownerroute.Router[giftStepHandoff, int64, playerroute.Route]{
		LocalSid:  owners.SID(),
		Routes:    owners.Routes(),
		Transport: ownerroute.NewBusTransport[giftStepHandoff](busClient, "game", giftHandoffModule),
		Executor:  steps.runHandoff,
...
// playerOwnership is the part of PlayerOwners these steps ask about: three
// questions, no claiming. Declared here rather than taking *PlayerOwners so
// the admission and handoff decisions can be tested without a Redis.
type playerOwnership interface {
	// Owns may claim a player nobody owns; it is the question a consumer
	// asks about work that has to happen somewhere.
	Owns(ctx context.Context, playerID int64) (bool, error)
	// OwnedHere never claims; it is the question asked about work that
	// belongs to whoever already has the player.
	OwnedHere(ctx context.Context, playerID int64) (bool, error)
...
type giftSteps struct {
	...
	// handoff sends a step to the process that owns its player. nil disables
	// forwarding, which leaves plain back-pressure: correct, just slower.
	handoff *ownerroute.Router[giftStepHandoff, int64, playerroute.Route]
}
...
func (steps giftSteps) runHandoff(ctx context.Context, cmd *giftStepHandoff) error {
	...
	// Check ownership again rather than trusting the address. The sender's
	// view of the route is a moment old, and running a step for a player who
	// has since moved is the corruption all of this exists to prevent.
	if steps.owners != nil {
		owned, err := steps.owners.OwnedHere(ctx, cmd.PlayerID)
		if err != nil {
			return fmt.Errorf("gift saga: handoff ownership of player %d: %w", cmd.PlayerID, err)
		}
		if !owned {
			// Not ours either. Do nothing: the holder of the message keeps
			// redelivering, and the next route lookup will be fresher.
			return nil
		}
	}
	// The phase decides which transaction runs, so a phase this process does
	// not know is refused BEFORE the claim — the same rule as admission. A
	// claim taken for work that cannot be done is a command nobody can run
	// until the lease expires.
	if cmd.Phase != giftHandoffPhaseDebit && cmd.Phase != giftHandoffPhaseRefund {
...
func (steps giftSteps) admitPhase(ctx context.Context, phase string, command saga.Command) error {
	...
	if !owned {
		...
		// handoff gets the work started on the owner NOW, and the refusal
		// keeps this message — unacknowledged — until a receipt exists. If
		// the handoff never arrives, the redelivery still converges on the
		// owner the slow way, which is why this is safe to be fire and
		// forget.
		steps.handOver(ctx, phase, state.From, command)
...
// handOver routes one step to the player's owner. Failures are logged and
...
// handoff at all.
func (steps giftSteps) handOver(ctx context.Context, phase string, playerID int64, command saga.Command) {
	if steps.handoff == nil {
		return
	}
	if err := steps.handoff.Route(ctx, &giftStepHandoff{PlayerID: playerID, Phase: phase, Command: command}); err != nil {
		slog.Warn("gift saga: step not handed over; redelivery will find the owner", "player_id", playerID, "phase", phase, "err", err)
	}
}
