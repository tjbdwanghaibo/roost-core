...
const (
	...
	// renews its lease and looks for a result to apply.
...
type ActivityRunner struct {
	...
	worldID     int64
	gameSID     int32
	// candidates is the set of game servers this deployment may run. The
	// coordinator's expected set is drawn from the ones that actually hold a
	// live lease, so an activity does not wait on a server nobody started.
	candidates []int32
	// now is the clock every window id is derived from. It is a field so a
	// test can put a contribution and a lookup on opposite sides of a window
	// boundary — which is the one case the wall clock makes unreproducible,
	// and the one that was failing in CI (RR-20260921-01).
	now func() time.Time
	// incarnation is this process's lease. A previous holder's incarnation
	// stops being able to renew, which is what makes "this sid is mine" a
	// fact rather than a convention.
	incarnation string
}

// startActivity builds the runner, takes this server's lease and starts the
...
func startActivity(ctx context.Context, registry *app.Registry) (*ActivityRunner, func(), error) {
	...
	gameSID := int32(config.GetInt("sid"))
	if gameSID <= 0 {
		return nil, nil, fmt.Errorf("activity: sid is %d; a server with no id cannot hold a lease", gameSID)
	}
	runner := &ActivityRunner{
		coordinator: coordinator, routing: routing, mailer: mailer, nest: nestClient,
		redis: redisClient, prefix: config.GetString("activity.key_prefix"),
		worldID: worldID, gameSID: gameSID,
		candidates: candidateSIDs(config.GetIntSlice("activity.game_sids"), gameSID),
		now:        time.Now,
	}
	if runner.prefix == "" {
		return nil, nil, errors.New("activity: activity.key_prefix is empty; the game reads the coordinator's keyspace to keep its own board beside it")
	}
	if err := runner.bindAndLease(ctx); err != nil {
		return nil, nil, err
	}
	...
	return runner, func() {
		...
		<-done
		<-done
		// Give the lease back rather than letting it lapse: a shutdown that
		// only stops renewing leaves this sid looking alive for a whole TTL,
		// and an activity opened in that window waits on a server that is
		// gone.
		releaseCtx, releaseCancel := context.WithTimeout(context.Background(), 3*time.Second)
		defer releaseCancel()
		if err := runner.routing.ReleaseLease(releaseCtx, runner.gameSID, runner.incarnation); err != nil {
			slog.Warn("activity: lease not released", "game_sid", runner.gameSID, "err", err)
		}
	}, nil
}

// candidateSIDs is the configured deployment, always including this server.
func candidateSIDs(configured []int, own int32) []int32 {
	out := []int32{own}
	for _, sid := range configured {
		if int32(sid) == own || sid <= 0 {
			continue
		}
...
// bindAndLease establishes this server's route and takes its lease.
func (runner *ActivityRunner) bindAndLease(ctx context.Context) error {
	...
	// never again.
	//
	// The refusal is recognised by CODE, not by errors.Is. This call crosses
	// the bus, and a remote error arrives as a coded error rather than as the
	// service's own sentinel (`remote.570110`), so a sentinel comparison
	// silently never matches — which is how "works the first time, refuses to
	// start the second" gets written.
	if _, err := runner.routing.Bind(ctx, runner.gameSID, gameactivity.GroupID, 1); err != nil && svcglobal.Code(err) != svcglobal.CodeConflict {
		return fmt.Errorf("activity: bind game %d: %w", runner.gameSID, err)
	}
	lease, err := runner.routing.AcquireLease(ctx, runner.gameSID)
	if err != nil {
		return fmt.Errorf("activity: acquire lease for game %d: %w", runner.gameSID, err)
	}
	runner.incarnation = lease.Incarnation
	slog.Info("activity: lease acquired", "game_sid", runner.gameSID, "incarnation", lease.Incarnation)
	return nil
}

// tickWorld drives the World's timer heap through a Nest transaction.
func (runner *ActivityRunner) tickWorld(ctx context.Context, done chan<- struct{}) {
	defer func() { done <- struct{}{} }()
	ticker := time.NewTicker(worldTickInterval)
	defer ticker.Stop()
	sender := syncsender.NewTickWorldTimersSender(runner.nest)
	for {
		select {
		case <-ctx.Done():
			return
		...
		if _, err := sender.Sync_TickWorldTimers(ctx, runner.worldID, time.Now().UnixMilli()); err != nil && ctx.Err() == nil {
...
func (runner *ActivityRunner) openCurrentWindow(ctx context.Context) {
	...
	key := gameactivity.Key(activityID)
	expected, err := runner.expectedGameSIDs(ctx)
	if err != nil {
		slog.Warn("activity: live games unavailable; window not opened", "activity_id", activityID, "err", err)
		return
	}
	// Already open is the normal answer on every pass but the first, and it
	// is recognised by code for the same reason Bind's conflict is.
	if _, err := runner.coordinator.OpenActivity(ctx, key, expected); err != nil && svcactivity.Code(err) != svcactivity.CodeExists {
		slog.Warn("activity: window not opened", "activity_id", activityID, "err", err)
		return
	}
...
// expectedGameSIDs is the candidates that hold a live lease. A server that is
// not running must not be waited on: the phase would sit collecting until the
// grace window expired, every window.
func (runner *ActivityRunner) expectedGameSIDs(ctx context.Context) ([]int32, error) {
	leases, err := runner.routing.LiveGames(ctx, gameactivity.GroupID, runner.candidates, len(runner.candidates))
	if err != nil {
		return nil, err
	}
	live := make([]int32, 0, len(leases))
	for _, lease := range leases {
		live = append(live, lease.GameSID)
	}
	if len(live) == 0 {
		// This server is running, whatever the lease store says. Opening an
...
func (runner *ActivityRunner) NotifyPhaseDue(ctx context.Context, activityID string) error {
	activityState, err := runner.coordinator.NotifyPhase(ctx, gameactivity.Key(activityID), runner.gameSID)
...
func (runner *ActivityRunner) Contribute(ctx context.Context, playerID int64, runID string) (string, svcactivity.Participant, error) {
	...
	participant, err := runner.coordinator.ApplyProgress(ctx,
		gameactivity.Key(activityID), strconv.FormatInt(playerID, 10),
	...
	if _, err := runner.redis.ZAdd(ctx, gameactivity.BoardKey(runner.prefix, activityID),
...
// not panic before it can be given one.
...
func (runner *ActivityRunner) applyDispatches(ctx context.Context) {
	owed, err := runner.coordinator.OwedDispatches(ctx, gameactivity.GroupID, runner.gameSID, activityDrainBatch)
	if err != nil {
...
func (runner *ActivityRunner) settle(ctx context.Context, activityID string, dispatch svcactivity.Dispatch) error {
	board, err := runner.redis.ZRevRangeWithScores(ctx,
		gameactivity.BoardKey(runner.prefix, activityID), 0, int64(gameactivity.PaidRanks-1))
	if err != nil {
	...
	recorded, err := syncsender.NewSettleActivitySender(runner.nest).
		Sync_SettleActivity(ctx, runner.worldID, activityID, time.Now().Unix())
	if err != nil {
		return fmt.Errorf("record settlement: %w", err)
	}
	if _, err := runner.coordinator.AckDispatch(ctx, gameactivity.Key(activityID), runner.gameSID, dispatch.Token); err != nil {
...
// renewLease is the heartbeat. The incarnation is required, so a process that
// lost its lease and did not notice cannot keep it alive.
func (runner *ActivityRunner) renewLease(ctx context.Context) {
	if _, err := runner.routing.RenewLease(ctx, runner.gameSID, runner.incarnation, nil); err != nil {
		slog.Warn("activity: lease not renewed", "game_sid", runner.gameSID, "err", err)
	}
}
...
func startActivityPhaseConsumer(ctx context.Context, registry *app.Registry, runner *ActivityRunner) (fnats.IJetStreamSubscription, error) {
	...
	database, prefix, stream := effectSettings(registry)
