...
// effectSettings reads the same keys the dataengine Mod publishes effects
// with, with the same defaults, so producer and consumer cannot disagree
// about where effects go.
func effectSettings(registry *app.Registry) (database, prefix, stream string) {
	database, prefix, stream = "game", "roost.effect", "ROOST_EFFECTS"
	cfg := registry.Config()
	if cfg == nil {
		return database, prefix, stream
	}
	if value := strings.TrimSpace(cfg.GetString("dataengine.database")); value != "" {
		database = value
	}
	if value := strings.Trim(strings.TrimSpace(cfg.GetString("dataengine.effects.subject_prefix")), "."); value != "" {
		prefix = value
	}
	if value := strings.TrimSpace(cfg.GetString("dataengine.effects.stream")); value != "" {
		stream = value
	}
	return database, prefix, stream
}
