...
14. **两个只有真的调用 `Serve` 才会暴露的缺陷**。✅ 已修。

    第 12 步做完时四仓全绿，但"全绿"漏掉了两件事，而且是同一类漏法。

    - **五个服务的进程起不来。** 拥有者 Mod 发布的是 capability **包装器**（这是消费方
      绑不到实现类型的原因），而 `Server.Service()` 把这个值交给手写的 `run` 钩子——
      钩子**必须**断言具体类型，因为它要调的正是那些**刻意不上总线**的拥有者专属方法
      （扫过期、重试发货、裁剪保留、推进宽限窗）。于是 `match`/`platform`/`chat`/
      `global/activity` 的 `Serve` 直接失败;`session` 那条是**写在 ticker 里的裸断言**,
      30 秒后 panic,而且只在真的配了 owner 的部署里 panic。

      没有任何测试抓到,因为**没有任何测试调用过 `Serve`**——只测了 `Init`,而 `Init`
      恰好是好的那半。改法:owner-only 名下注册**未包装的实现本身**。这不削弱消费方
      的保证——owner-only 名只存在于拥有者进程,消费方查它在拆分部署下什么都查不到,
      在 lookup 处大声失败而不是在类型断言处安静失败。

    - **`servicerpc -check` 根本不检查生成物。** 它在拒绝规则跑完后就 `return nil`,
      对过时的、被手改的、或由另一个版本生成器产出的文件一律退出 0——而它在两份 README
      里被写成 CI 的漂移门禁。一个不可能失败的门禁比没有门禁更坏。现在它生成并逐字节
      比对，并区分"缺失"与"不一致"。

    两条都是**第 12 步那个教训的重演**:那一步发现四个包里断言具体类型的集成子测试
    长期假绿,原因是"只为一个包写了那条测试"。这里同样是"只为一个包写了 `Init` 的测试",
...
15. **运维面 `admin.go`**。✅ 已完成，三个服务。
    ...
    - `mail` 满邮箱是有文档的上界，`chat` 超期未裁剪是存储成本，`activity` 的 audit
      溢出有计数器。都不是"没有任何路可走"。

    `session` 那条的后果我一开始也说反了。写的是"owner 不会被挡，因为 `Run.Live` 不看
    pending 资源"——挡住 `Enter` 的是 **claim**，不是 `Live`：`Enter` → claim 被占 →
    `resolveClaim` → run 不 live → 释放资源失败 → **claim 永不释放**。claim 是故意在
    清理成功之后才释放的（第八节那条"claim 绝不能比清理活得更久"），代价就是一个永远
    无法成功的释放 = 一个**永远进不去的玩家**。这比"外部资源泄漏"严重得多。

    三条共同的约束，每条都对应本文第 3.9 节的一个模式：

    - **不上总线**（都没有 `//roost:rpc`）。这三个操作比各自接口上的任何方法都危险，
      而总线不带这些服务能验证的调用方身份——正是第 3.9 节"身份/权限来自客户端"那条。
      只有拥有者进程摸得到；人怎么摸到那个进程是部署决策，做在凭据所在的地方。
      集成测试双向钉住：公开 capability **不能**满足 `Admin`，owner-only capability
      **必须**满足（否则运维面存在但没人能调，就是死路本身）。
    - **note 必填无默认**——"零可观测性"那条。一次在已付款订单上、没有记录理由的干预
      是无法复核的。
    - **不做枚举**。这是对规划的另一处更正："找不到耗尽的订单"听起来像问题，其实支付
      渠道手里有权威清单，运维真正问的是"渠道说收了钱的这些单，我们发货了吗"，
      `Service.Order` 已经按 id 回答。在这里建索引是重复一份本服务不拥有的事实来源，
      而且必须写在设置终态那次 CAS 之外，于是它可以和它索引的记录不一致。

    两条最容易反过来做错、都对应已有的设计规则：**重开时 ACK 令牌不换**（game 可能已
    应用结果只是确认丢了，唯一能去重的键就是令牌——mail claim token 那条规则）；
    **`settled` 是第三个终态而不是复用 `delivered`**（否则一次把人工退款算成已发货的
    对账，会报出本服务并没有完成的履约）。

    22 条变异验证，其中**六条一开始是绿的**，全是测试自身的问题。最值得记的一条连续
    两次绿："顺手调一次 Releaser 并忽略错误"——第一次因为 fake 只数成功不数调用，第二次
    因为我在干预**之后**才读基线，那次多出来的调用已经算进基线了。
