...
| M-06 | core（kit 半待 core v1.15.3 发版） | match 领域实现（类型、errcode 段、Store 状态机、Redis store、Grouping）与 `servicemetrics` 契约下沉 core：`roost-core/service/match`、`roost-core/servicemetrics`；kit 侧改别名包的步骤写在记录里 | [M-06-match-domain-into-core.md](M-06-match-domain-into-core.md) |
