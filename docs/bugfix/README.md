...
| ARCH-08 | room 同步的分层收敛：A. `RoomEnvelopeSink` 每房私有却按 roomID 分片键（收成私有形状或改为 manager 级共享）；B. `SubscriptionCoordinator` 单消费者、room 重复记账、三层各一套失败政策（并回 room 或补第二个消费者） | **待拍板**（2026-09-22 第二轮分流 W-2026-09-22-04 / -05；[报告](../bug/REVIEW-2026-09-22-02.md)） |
| ARCH-09 | 非房间的实体复制路径与 `syncTopic` 的去向：`sync=true` 只得到状态机 + packer，调度 / 订阅 / 传输全在 room；`Topic` 只写成 Namespace、无路由。补非房间调度器（同时是 ARCH-08 B 的第二个消费者）或收窄承诺；无 notifier 告警可先行 | **待拍板**（2026-09-22 第二轮分流 W-2026-09-22-06） |
