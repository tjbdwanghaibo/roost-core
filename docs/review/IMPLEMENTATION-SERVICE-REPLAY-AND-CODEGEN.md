...
## 2026-09-10 补充：token 稳定性受 Entry 保留期限制

Kit c4e7cef1029fb6326fa88b84c622f059bc62c0c4 的 Deliver/evict/ReserveClaim 组合独立复现了 [RR-20260910-02](../bug/REVIEW-2026-09-10-02.md)：容量淘汰删除终态 Entry，也删除 token；相同信封重投后会生成不同 token。此前关于 token 稳定的描述只对仍保存的 Entry 成立，不能外推到淘汰重建之后。

去重记录必须覆盖业务允许的重放窗口。展示历史可以淘汰，不代表领取身份也可忘记。本轮证明新 token 再次可领，未执行真实发奖；如果资产侧仅按该 token 去重，就存在重复发放风险。状态墓碑、稳定业务身份及重放期限是建议方向，尚未实现。
