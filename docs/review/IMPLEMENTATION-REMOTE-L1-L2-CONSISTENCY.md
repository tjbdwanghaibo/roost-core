...
09-14 真实 Redis：A 删除 v2 留本地墓碑，B 通过独立缓存重写 v1，L2 接纳；A 自己仍拒绝读取该值。删除版本比较只能保护当前仍存在的 L2 值，DEL 后比较依据消失。全局删除需要共享水位或代际准入，TTL 必须与可重放窗口关联；性能代价是额外持久状态和清理协议，本轮未压测。[工作项与证据](OPEN-QUESTIONS.md)。
