...
| core | `entity` | 09-02 | 09-06 U-0065（回退 7 条） / 09-07 U-0099（回退 16 条） / 09-09 U-0148 / 09-13 U-0174（修复） | 09-02 | 09-02 | 09-02 | 09-02 | 09-02 | 09-02 / 09-13 U-0175（修复） |
...
| kit | `remoteentity`（新位置 core） | 09-02 | 09-05 U-0023（真实 Mongo） / 09-06 U-0049（回退 40 条） / 09-06 U-0093（回退 8 条） / 09-09 U-0128（回退 12 条） / 09-13 U-0179（修复） | 09-02 | 09-04 U-0011 / 09-06 U-0025 | 09-02 | 09-02 | 09-02 | 09-05 U-0023 / 09-10 U-0168（修复） / 09-11 U-0173（修复） / 09-13 U-0178（修复） / 09-13 U-0176、U-0177（修复） |
...
| U-0175 | 2026-09-13 | roost-core `entity` 快照读取准入与绝对有效期(RR-20260913-08) | C8 | 信封有 `ExpiresAt` 与 `Expired()`,但缓存只用固定容器 TTL,`Get` 从不看它:ExpiresAt 在过去、TTL 还剩一分钟时仍然命中,返回一份自己声明已过期的快照。修法:`Get` 命中后判一次 Expired,过期即降级为未命中 —— Cached 答没有,Monotonic 按契约回权威 | `snapshot_expiry_promises_test.go`(过期不命中、Monotonic 回权威且只问一次、无 ExpiresAt 的照常命中) | 修前红(`served a snapshot that declares itself expired`);修后 entity 含 -race 全绿 | T-69 · `docs/bugfix/RR-20260913-08.md` |
