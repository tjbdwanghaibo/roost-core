...
| U-0187 | 2026-09-13 | roost-core `entity` + `remoteentity` 快照删除的版本屏障(RR-20260913-01) | C8 | 复制侧 `cache.Delete(key)` 不看 `env.Version` 也不留水位,迟到的 delete v2 清掉 v3、先到的 delete v2 之后 full v1 复活。修法:`DeleteAtVersion` 与 `Publish` 共用分片锁,比 L1 新的删除不动,否则删并留 `TombstoneTTL`(默认=缓存 TTL)内的墓碑;Publish 丢弃 <= 墓碑版本的写、更新的清墓碑;复制接收与提交后失效都按版本删 | `snapshot_delete_order_promises_test.go`(真实 ReplicaStore 重排投递)、`snapshot_delete_version_promises_test.go`(缓存层含墓碑到期边界) | 修前红(`old delete removed v3` / `deleted snapshot resurrected`);修后 entity / remoteentity 含 -race 全绿 | T-81 · `docs/bugfix/RR-20260913-01.md` |
...
| U-0184 | 2026-09-13 | roost-core `remoteentity` 兴趣订阅的代际(RR-20260913-02) | C8 | 接收端 release 按 key/SID 无条件删,迟到的旧 release 取消新 renewal;ExpiresAt 不能当版本(合法 release 早于 lease 到期)。修法:`RemoteSnapshotInterest.Generation` 只增(计数器从 UnixNano 播种,重启复用 SID 不倒退),注册表记 `{expiresAt, generation}`,renew 忽略落后代际,release 只删不落后的 | `interest_generation_promises_test.go`(注册表层交错 + Manager 层递增与重启) | 修前红(`an older release canceled the newer renewal`);修后 -race 全绿 | T-78 · `docs/bugfix/RR-20260913-02.md` |
...
| U-0180 | 2026-09-13 | roost-core `entity` 发布时 L2 冲突的可见性(RR-20260913-05) | C8 | `IgnoreRemoteError` 把 L2 的版本冲突当可用性故障吞掉,Publish 返回 nil,L1=B/L2=A。修法:L1 未命中时先有界地问 L2 一次,同版本不同内容即 `ErrRemoteVersionConflict` 不落写;L2 读失败仍按故障降级;同版本比较抽成共用函数 | `snapshot_l2_conflict_promises_test.go` 的 publish 用例(含 L2 故障仍降级的对照) | 修前红(`= <nil>, want ErrRemoteVersionConflict`);修后 entity 含 -race 全绿 | T-74 · `docs/bugfix/RR-20260913-05.md` |
