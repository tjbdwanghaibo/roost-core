...
| 编号 | 等级 | 问题 | 状态 |
| --- | --- | --- | --- |
| RR-20260921-01 | P2 | 贡献落进哪个窗口既不返回也不记录，机器人只能对"读的时候恰好是哪个窗口"下断言，跨 300 秒边界必然红 | 已修复（U-0273，未发版）→ [bugfix](../bugfix/RR-20260921-01.md) |
| RR-20260921-02 | P2 | demo-publish 从未成功过：发布的树自带 `.github/workflows/`，Actions 的 token 不被允许推送 workflow 文件 | 已修复（U-0274，未发版）→ [bugfix](../bugfix/RR-20260921-02.md) |
