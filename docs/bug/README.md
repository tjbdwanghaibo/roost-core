...
| 编号 | 等级 | 问题 | 状态 |
| --- | --- | --- | --- |
| RR-20260920-11 | P1 | `confirm` 只在 `lastUsed` 为零时盖章，为新工作重新取得的租约带着旧时间戳，下一轮就被当成空闲连同实体一起还掉——还在跑的步骤脚下被抽空 | 已修复（U-0271，未发版）→ [bugfix](../bugfix/RR-20260920-11.md) |
| RR-20260920-12 | P2 | `evictBudget` 是 ctx 超时，而 `EntityManager.Destroy` 等实体锁时不读 ctx；一个忙实体能钉住整轮刷新，后面所有玩家的续租排队 | 已修复（U-0272，未发版）→ [bugfix](../bugfix/RR-20260920-12.md) |
