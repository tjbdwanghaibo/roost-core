...
// AttributeComponent owns the player's attribute container.
//
...
func init() {
	entity.RegisterComponentFactory(CompTypeAttribute, func(owner any, _ *entity.EntityCreateParam) (entity.ComponentInterfaceBase, error) {
		...
		return &AttributeComponent{owner: typed, container: coreattr.NewContainer()}, nil
	})
}
...
// OnInitFinish is where the container is filled, and it runs at exactly the
// right moment: the generated builder attaches the DAO before it initializes
// components, so the stored values are already here. A new player has none
// and starts from the level curve.
func (component *AttributeComponent) OnInitFinish(_ *entity.EntityCreateParam, isCreate bool) error {
	dao := component.owner.Dao()
	base := &gameattr.Combat{}
	if isCreate || dao.AttrBaseLen() == 0 {
		base = gameattr.ForLevel(dao.GetLevel())
	} else {
		values := make(map[coreattr.AttrID]coreattr.AttrValue, dao.AttrBaseLen())
		dao.RangeAttrBase(func(id int32, value int64) bool {
			values[coreattr.AttrID(id)] = coreattr.AttrValue(value)
			return true
		})
		base.LoadValues(values)
	}
	component.container.Install(coreattr.Base, base)
	component.container.Install(Gear, &gameattr.Combat{})
	component.recompose()
	return nil
}

// Container is the layered view, for callers holding the entity lock.
func (component *AttributeComponent) Container() *coreattr.Container {
	return component.container
}

// Final is a copy of the composed profile: safe to read after the lock is
// released, which a snapshot is exactly for.
func (component *AttributeComponent) Final() (*gameattr.Combat, bool) {
	return gameattr.CombatIn(component.container, coreattr.Final)
}

// LevelUp raises the base layer and recomposes. It is the attribute half of
// ProfileComponent.AddExp: the level is the DAO's, the numbers that follow
// from it are the container's.
...
func (component *AttributeComponent) RefreshGear() {
	...
	attack, hp := int64(0), int64(0)
	// From what is WORN, not from the whole bag: carrying a sword in the bag
	// should not make a player stronger, and the earlier version of this
	// counted every stack (there was nowhere else to read equipment from).
	...
	if gear.Attack == attack && gear.HP == hp {
		return
	}
...
// recompose rebuilds Final from every other layer.
func (component *AttributeComponent) recompose() {
	sum := make(map[coreattr.AttrID]coreattr.AttrValue)
	for _, selector := range component.container.Layers() {
		if selector == coreattr.Final {
			continue
		}
		profile, ok := component.container.Live(selector)
		if !ok {
			continue
		}
		...
		for id, value := range values {
			...
			if meta, ok := profile.MetaByID(id); ok && meta.Derived {
				continue
			}
			sum[id] += value
		}
	...
	component.container.Install(coreattr.Final, final)
}

// persist writes the two layers the DAO carries and hands the change to the
// replication subject. Base is stored and replicated; Final is replicated
// only (the DAO field is nopersist), which is why one call does both.
func (component *AttributeComponent) persist() {
	dao := component.owner.Dao()
	if base, ok := gameattr.CombatLive(component.container, coreattr.Base); ok {
		writeAttrMap(base, dao.SetAttrBase)
	}
	if final, ok := gameattr.CombatLive(component.container, coreattr.Final); ok {
		writeAttrMap(final, dao.SetAttrFinal)
	}
	component.owner.PublishSyncDirty()
}

func writeAttrMap(profile coreattr.Profile, set func(int32, int64)) {
	values := make(map[coreattr.AttrID]coreattr.AttrValue)
	profile.ExportValues(values)
	for id, value := range values {
