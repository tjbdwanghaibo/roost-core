...
// A deadline already in the past when the process starts still fires, rather
// than being refused for being late: the window it closes is the one nobody
// closed while the process was down.
func TestADeadlineThatIsAlreadyPastStillFires(t *testing.T) {
	subject := newWorld(t)
	committer := &worldCommitter{}
	now := time.Unix(1_800_000_000, 0)

	if _, err := nest.RunIsolatedTransaction(context.Background(), committer, "arm_late", func() (any, error) {
		if _, armed := subject.TimerComp().ScheduleActivityPhase("race-1799999000", now.Add(-time.Hour), now); !armed {
			t.Fatal("a deadline in the past was refused instead of armed")
		}
		return nil, nil
	}); err != nil {
		t.Fatalf("arm: %v", err)
	...
	if _, err := nest.RunIsolatedTransaction(context.Background(), committer, "tick", func() (any, error) {
		if remaining := subject.TimerComp().Tick(now); remaining != 0 {
			t.Errorf("the overdue deadline did not fire: %d nodes remain", remaining)
		}
		return nil, nil
	}); err != nil {
		t.Fatalf("tick: %v", err)
