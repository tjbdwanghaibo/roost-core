...
//
//   - **It survives a restart.** The heap is rebuilt from the DAO on load, so
//     "close the activity at 20:00" is still due after a deploy at 19:58. An
//     in-process timer loses every pending deadline with the process, and
//     loses it silently.
...
// OnInitFinish rebuilds the heap from what was stored. It runs after the DAO
// is attached and before anything can call Tick, which is the only order in
// which a restored deadline is not briefly invisible.
//
// Nothing here writes: restoring a node must not mark the World dirty, or
// every load would produce a save of state that did not change.
func (component *TimerComponent) OnInitFinish(_ *entity.EntityCreateParam, _ bool) error {
	...
	dao.RangeTimers(func(id int64, stored *db.TimerNode) bool {
		...
		saved = append(saved, timer.Node{
			ID: id, Type: stored.GetType(), Param1: stored.GetParam1(), Param2: stored.GetParam2(),
	...
	component.scheduler = timer.NewScheduler(component.owner.ID(), dao.GetTimerSeed(), saved, component.persist)
	component.scheduler.RegisterHandler(TimerTypeActivityPhase, component.onActivityPhase)
	return nil
}
...
// pin makes the scheduler read the caller's time instead of the wall clock.
//
// Both entry points already carry the moment they act at — the tick carries
// the tick's time, arming carries the transaction's — and letting the
// scheduler read time.Now() inside them would date a deadline by when the
// entity's lock happened to be acquired rather than by the tick that armed
// it. Pinned, the World's schedule is a function of what it was told, which
// is also what makes it testable without waiting.
func (component *TimerComponent) pin(now time.Time) {
	component.scheduler.SetClock(func() time.Time { return now })
}
...
// idempotent per game sid — but a heap that grows by one node per restart is
// not.)
func (component *TimerComponent) ScheduleActivityPhase(activityID string, at time.Time, now time.Time) (int64, bool) {
	if activityID == "" || component.scheduler == nil {
		return 0, false
	}
	...
	return component.scheduler.NewTimer(delay, TimerTypeActivityPhase, 0, 0, []byte(activityID)), true
}
...
func (component *TimerComponent) PendingActivityPhase(activityID string) (int64, bool) {
	...
	for _, node := range component.scheduler.Nodes() {
		if node.Type == TimerTypeActivityPhase && string(node.Payload) == activityID {
			return node.ID, true
		}
	}
	return 0, false
...
func (component *TimerComponent) Tick(now time.Time) int {
	...
	component.pin(now)
	component.scheduler.Tick(now)
	return len(component.scheduler.Nodes())
}
