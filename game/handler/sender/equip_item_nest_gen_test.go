...
// TestEquipItemSenderRefusesCallsWithoutAClient pins that a sender constructed without a
// Nest client, or a nil sender, refuses with nest.ErrNestStopped instead of
// dereferencing nil on the first call.
func TestEquipItemSenderRefusesCallsWithoutAClient(t *testing.T) {
	var none *EquipItemSender
	if _, err := none.nestClient(); !errors.Is(err, nest.ErrNestStopped) {
		t.Fatalf("nil sender nestClient() = %v, want nest.ErrNestStopped", err)
	}
	if _, err := NewEquipItemSender(nil).nestClient(); !errors.Is(err, nest.ErrNestStopped) {
