...
// 构造时就拒绝，不能等到运行起来才发现。
//
// 旧行为：validate 只查半径与分带，BlockIndex 只拦整图格数，而
...
func TestObserverBlockBudgetRefusesARatioThatSubscribesTheWholeMap(t *testing.T) {
	config := InterestConfig{
		Bounds:      Rect{Max: Point{X: 10000, Y: 10000}},
		BlockSize:   10,
		EnterRadius: 900,
		LeaveRadius: 1000,
	}
	if _, err := NewInterestManager(config); !errors.Is(err, ErrInterestBudget) {
...
func TestObserverBlockBudgetIsConfigurable(t *testing.T) {
	...
	if _, err := NewInterestManager(config); err != nil {
...
func TestTheNineBlockShapeFitsTheDefaultBudget(t *testing.T) {
	manager, err := NewInterestManager(InterestConfig{
		Bounds:      Rect{Max: Point{X: 10000, Y: 10000}},
		BlockSize:   150,
		EnterRadius: 120,
		LeaveRadius: 150,
	...
	if err := manager.AddObserver(1, Point{X: 5000, Y: 5000}); err != nil {
...
func TestObserverBlockBudgetSaturatesInsteadOfWrapping(t *testing.T) {
	config := InterestConfig{
		Bounds:      Rect{Max: Point{X: 1 << 40, Y: 1 << 40}},
		BlockSize:   1,
		EnterRadius: 1 << 40,
		LeaveRadius: 1 << 41,
	}
